...
for feature in data["features"]:
    ...
    wkt_coords = "MULTIPOLYGON(((" + "), (".join([", ".join([f"{lon} {lat}" for lon, lat in coord]) for coord in geometry["coordinates"][0]]) + ")))"
    wkt = f"MULTIPOLYGON((({wkt_coords})))"
    g.add((uri, GEO.asWKT, Literal(wkt, datatype=GEO.wktLiteral)))

g.serialize(destination="departementen3.ttl", format="turtle")
