...
# Functie om API aan te roepen en gegevens op te halen inclusief geometrie
def fetch_data_from_api(gemeente_uri, sectie, perceelnummer):
   ...
   #Monnr.;perceel;sectie;gemeentenaam;origineleGemeentecode;GemeentecodeUri;cbscode 
    url = f"https://api.linkeddata.cultureelerfgoed.nl/queries/rce/Percelen-bij-kadaster-1/6/run?sectie={sectie}&perceelnummer={perceelnummer}&gemeente={gemeente_uri}"
...
def main():
    ...
    input_csv_path = r"C:\Users\linkeddata\Downloads\Albert\torens en kadastraal perceel_versie2c.csv"
    
    # Pad voor het nieuwe CSV-bestand
    output_csv_path = r"C:\Users\linkeddata\Downloads\Albert\torens en kadastraal perceel_versie2_geometrie.csv"
    ...
    for index, row in df.iterrows():
        ...
        if api_data:
            ...
            geometrie_list.append(api_data.get('shape', ''))
        else:
            oppervlakte_list.append('')
            geometrie_list.append('')
    ...
    # Toevoegen van oppervlakte en geometrie aan het oorspronkelijke dataframe
    df['oppervlakte'] = oppervlakte_list
    df['geometrie'] = geometrie_list
