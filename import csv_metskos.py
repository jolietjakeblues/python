...
def download_and_combine_rdf(input_csv_path, output_rdf_path):
    # Namespaces for parsing RDF/XML
    namespaces = {
    ...
    rdf_root = ET.Element("rdf:RDF", xmlns=namespaces['rdf'])
    ...
    for url in tqdm(urls, desc="Downloading and combining RDF/XML data", unit="url"):
        try:
            ...
            # Extract prefLabels and link them to their subjects
            for elem in tree.findall('rdf:Description', namespaces):
                subject = elem.get('{http://www.w3.org/1999/02/22-rdf-syntax-ns#}about')
                label = elem.find('skos:prefLabel', namespaces)
                if label is not None:
                    prefLabels[subject] = label.text
    ...
    # Here you can do something with the prefLabels dictionary or save it somewhere
